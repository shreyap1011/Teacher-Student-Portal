<?php
    ...
    $stmt8 = "select * from `examContentTable` where `examId` = '$examId'";
    $query8 = mysqli_query($connection,$stmt8);
    $Pscore = [];
    while($row = mysqli_fetch_array($query8)){
        array_push($Pscore, $row['score']);
    }
    array_push($info, array('Pscore' => $Pscore));
    ...
    $stmt8 = "select * from `commentTable` where `examId` = '$examId'";
    $query8 = mysqli_query($connection,$stmt8);
    $comment = [];
    while($row = mysqli_fetch_array($query8)){
        array_push($comment, $row['comment']);
    }
    array_push($info, array('comment' => $comment));
