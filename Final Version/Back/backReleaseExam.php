...
for($i = 0; $i < count($questionId); $i++){
    $testcase = 0;
    $stmt6 = "select * from `Question_Bank` where `questionId` = '$questionId[$i]'"; 
    $query6 = mysqli_query($connection,$stmt6);
    while($row = mysqli_fetch_array($query6)){
        $testcase = $row['totalTestCase'];
    }
    for($j = 0; $j < $testcase; $j++){
        $count = $j + 1;
        $stmt5 = "UPDATE studentExamResult SET score = '$score[$t]' WHERE examId = '$examId' AND questionId = '$questionId[$i]' AND testcaseId = '$count'";
...
for($i = 0; $i < count($questionId); $i++){
    ...
    $stmt6 = "UPDATE nameCheck SET score = '$fscore[$i]' where examId = '$examId' AND questionId = '$questionId[$i]'";
    $query6 = mysqli_query($connection,$stmt6);
    if(!$query6){
        $positive = false;
        break;
    }
    $stmt7 = "UPDATE constraintCheck SET score = '$cscore[$i]' where examId = '$examId' AND questionId = '$questionId[$i]'";
